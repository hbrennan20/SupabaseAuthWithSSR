    ...
    <Container maxWidth="xl">
      ...
      <Grid container spacing={3}>
        ...
        <Grid item xs={12} md={4}>
                  ...
                  <Box key={type} sx={{ display: 'flex', alignItems: 'center' }}>
                    <Box 
                      sx={{ 
                        ...
                        height: 16, 
                        borderRadius: '50%', 
                        backgroundColor: type === 'Shot' ? 'blue' : type === 'Pass' ? 'green' : type === 'Tackle' ? 'orange' : 'red',
                        mr: 1 
                      }} 
                    />
          ...
          <Paper elevation={3} sx={{ p: 2, mb: 2 }}>
            <Typography variant="h6" gutterBottom>Summary Stats</Typography>
            <Box component="table" sx={{ width: '100%', borderCollapse: 'collapse' }}>
              <tbody>
                {['Shot', 'Tackle', 'Kickout', 'Turnover'].map((type) => (
                  <Box component="tr" key={type} sx={{ '&:not(:last-child)': { borderBottom: '1px solid rgba(224, 224, 224, 1)' } }}>
                    <Box component="td" sx={{ py: 1 }}>Total {type}s</Box>
                    <Box component="td" sx={{ py: 1, textAlign: 'right' }}>{tags.filter(tag => tag.type === type).length}</Box>
                  </Box>
                ))}
              </tbody>
            </Box>
          ...
          {selectedTag && (
            <Paper elevation={3} sx={{ p: 2 }}>
              <Typography variant="h6" gutterBottom>Selected Tag Type</Typography>
              <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1 }}>
                {['Shot', 'Pass', 'Tackle', 'Turnover'].map((type) => (
                  <button 
                    key={type}
                    onClick={() => handleTagTypeChange(type as 'Shot' | 'Pass' | 'Tackle' | 'Turnover')}
